...
namespace AspNetCoreID.Controllers
{
                ...
                if (user != null && !await userManager.IsLockedOutAsync(user))
                {
                    if (await userManager.CheckPasswordAsync(user, model.Password))
                    {
                        ...
                        if (await userManager.GetTwoFactorEnabledAsync(user))
                        {
                            ...
                            //{
                            //    await HttpContext.SignInAsync(IdentityConstants.TwoFactorUserIdScheme, Store2FA(user.Id, userManager.Options.Tokens.AuthenticatorTokenProvider));
